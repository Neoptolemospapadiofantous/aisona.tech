...
const Contact = () => {
  ...
  return (
    ...
    >
      ...
      <div className="max-w-7xl mx-auto px-4 py-8 sm:py-12 pt-20 sm:pt-32 relative z-10">
        {/* Header */}
        <div className={`text-center mb-12 sm:mb-16 transition-all duration-1000 transform ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-6 max-w-3xl mx-auto">
            {trustSignals.map((signal, index) => (
              <div key={index} className="text-center bg-white rounded-xl p-3 sm:p-4 shadow-sm border border-gray-100 hover:shadow-md transition-all duration-300">
                <div className="text-xl sm:text-2xl font-bold text-blue-600 mb-1">{signal.value}</div>
                <div className="text-xs sm:text-sm text-black">{signal.label}</div>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className={`mb-12 sm:mb-16 transition-all duration-1000 delay-300 transform ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
          <div className="bg-white border border-blue-200 rounded-2xl shadow-2xl p-6 sm:p-8 max-w-4xl mx-auto transition-all duration-500 hover:shadow-3xl hover:scale-105 group relative overflow-hidden">
            ...
            <div className="relative z-10">
              <div className="flex flex-col sm:flex-row items-center justify-center gap-3 mb-4">
                <div className="w-10 h-10 sm:w-12 sm:h-12 bg-blue-600 rounded-full flex items-center justify-center shadow-2xl">
                  <MessageSquare className="w-5 h-5 sm:w-6 sm:h-6 text-white" />
                </div>
                <h2 className="text-xl sm:text-2xl font-bold text-black transition-all duration-300 group-hover:text-blue-700 text-center sm:text-left">
                  Instant AI Support Available
                </h2>
              </div>
              <p className="text-black mb-4 sm:mb-6 max-w-2xl mx-auto transition-colors duration-300 group-hover:text-gray-700 text-center text-sm sm:text-base">
              ...
              <div className="bg-blue-50 rounded-lg p-3 sm:p-4 border border-blue-200">
                <div className="text-blue-600 font-medium flex items-center justify-center gap-2 text-sm sm:text-base">
                  <span className="w-2 h-2 sm:w-3 sm:h-3 bg-green-500 rounded-full animate-pulse block"></span>
        ...
        <div className={`grid lg:grid-cols-3 gap-4 sm:gap-6 mb-12 sm:mb-16 transition-all duration-1000 delay-500 transform ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
          {contactMethods.map((method, index) => (
            <div
              key={index}
              className={`rounded-2xl p-4 sm:p-6 border transition-all duration-300 hover:shadow-xl hover:scale-105 group relative overflow-hidden ${
                method.highlight 
            ...
            >
              ...
              <p className="text-black text-sm mb-3 sm:mb-4 leading-relaxed">
                {method.description}
              </p>
              ...
              {method.href ? (
                <a
                  href={method.href}
                  className="text-blue-600 hover:text-blue-700 font-medium text-sm transition-all duration-300 inline-flex items-center gap-2 group/link min-h-[44px] items-center"
                >
                  {method.action}
                ...
                <button 
                  ...
                  className="bg-blue-600 hover:bg-blue-700 text-white font-medium py-3 px-6 rounded-lg text-sm transition-all duration-300 w-full shadow-md hover:shadow-lg transform hover:-translate-y-1 min-h-[48px]"
                >
                  {method.action}
                </button>
              )}
        ...
        <div className="max-w-6xl mx-auto mb-12 sm:mb-16">
          <div className={`grid lg:grid-cols-2 gap-6 sm:gap-8 transition-all duration-1000 delay-700 transform ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
            {/* Business Hours */}
            <div className="bg-white border border-gray-200 rounded-2xl shadow-lg p-6 sm:p-8 hover:shadow-xl transition-all duration-300">
              <h3 className="text-xl font-bold text-black mb-4 sm:mb-6 flex items-center gap-3">
              ...
              <div className="space-y-3 sm:space-y-4">
                {businessHours.map((schedule, index) => (
                  <div key={index} className={`flex flex-col sm:flex-row sm:justify-between sm:items-center py-2 sm:py-3 px-3 sm:px-4 rounded-lg transition-all duration-300 gap-1 sm:gap-2 ${
                    schedule.highlight 
                      ? 'bg-blue-50 border border-blue-200' 
                      : 'hover:bg-gray-50'
                  }`}>
                    <span className="text-black font-medium text-sm sm:text-base">{schedule.day}</span>
            ...
            <div className="bg-white border border-gray-200 rounded-2xl shadow-lg p-6 sm:p-8 hover:shadow-xl transition-all duration-300">
              ...
              <div className="grid gap-3 sm:gap-4">
                {whyChooseUs.map((item, index) => (
                  <div key={index} className="flex items-start gap-3 sm:gap-4 p-2 sm:p-3 rounded-lg hover:bg-blue-50 transition-all duration-300 group">
                    <div className="w-8 h-8 sm:w-10 sm:h-10 bg-blue-100 rounded-xl flex items-center justify-center flex-shrink-0 transition-all duration-300 group-hover:bg-blue-200 group-hover:scale-110">
                      <item.icon className="w-4 h-4 sm:w-5 sm:h-5 text-blue-600" />
                    </div>
                    <div className="flex-1">
                      <h4 className="font-semibold text-black mb-1 text-sm sm:text-base transition-colors duration-300 group-hover:text-blue-700">
                        {item.title}
                      </h4>
